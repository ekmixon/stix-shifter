...
class ElasticQueryStringPatternTranslator:
    ...
    def _format_set(values) -> str:
        ...
        return f"({' OR '.join([ElasticQueryStringPatternTranslator._escape_value(value) for value in gen])})"
    ...
    def _format_equality(value) -> str:
        ...
        return f'{value}' if (type(value) is datetime.datetime) else f'"{value}"'
    ...
    def _parse_expression(self, expression) -> str:
        if isinstance(expression, ComparisonExpression):  # Base Case
            ...
            mapped_field = f"{self.fields_prefix}{self.dmm.map_field(stix_object, stix_field)}"

            scope_to_object = f"{self.object_prefix}:{mapped_object}"
            ...
            return f"({self._parse_expression(expression.expr1)}) {self.comparator_lookup[expression.operator]} ({self._parse_expression(expression.expr2)})"
