...
class RelevanceQueryStringPatternTranslator:
    ...
    def _lookup_comparison_operator(self, expression_operator):
        ...
        if expression_operator not in self.comparator_lookup:
            raise NotImplementedError(
                f"Comparison operator {expression_operator.name} unsupported for BigFix adapter"
            )
                ...
                if key not in ('date', 'ipv4_cidr', 'domain_name') and bool(re.search(pattern, each)):
                    if value_type and key not in value_type:
                        if not reduce(op.and_, [each in combinable_value_attr for each in value_type]) or \
                            ...
                            raise ValueError(f"Cannot combine {key} with {value_type} for IN operation")
                    else:
    ...
    def _map_transformer_to_field(value, comparator) -> str:
        ...
        if isinstance(value, list):
            ...
            transformer = _transformer_hierarchy[
                max(
                    _transformer_hierarchy.index('as lowercase')
                    if each.replace('"', '').isalpha()
                    else _transformer_hierarchy.index('')
                    if each.replace('"', '').isdigit()
                    else _transformer_hierarchy.index('as string')
                    for each in value
                )
            ]
    ...
    def _query_const_from_list_subroutine(expr_list):
        ...
        for index, each_exp_list in enumerate(expr_list):
            interim_list.extend(iter(each_exp_list))
            if index == len(expr_list) - 1:
                interim_str = f"({' OR '.join(interim_list)})"
        return interim_str
    ...
    def _query_construction_from_list(self, value, comparison_string_list, conditional_attr):
        ...
        if comparison_string_list:
            if conditional_attr:
                ...
                comparison_string = f"({' OR '.join(master_qry_list)})"
            else:
    ...
    def _relevance_qry_list_phrasing(self, value, comparator, mapped_field, is_negate):
        ...
        mapped_field = (
            f"{mapped_field} of " if self._relevance_string_list else mapped_field
        )

        mapped_field_relevance_string = "{} {} of it".format(mapped_field, ' of '.join(
            self._relevance_string_list)).lstrip()
        format_string_key = operator_mapping.get(
            comparator, operator_mapping.get('default')
        )
    ...
    def get_master_obj_of_obs_exp(self, expression, objects_list):
        ...
        if hasattr(expression, 'object_path'):
            ...
            if current_comparison_obj in self._relevance_property_format_string_dict.get('object_hierarchy'):
                ...
                elif not self._master_obj or objects_list.index(current_comparison_obj) > \
                                objects_list.index(self._master_obj):
                    self._master_obj = current_comparison_obj
            elif expression.object_path in object_marked_for_expr_split:
                self._split_master_obj_list.append(ADAPTER)
        else:
    ...
    def _parse_time_range(qualifier, stix_obj, relevance_map_dict, time_range, range_operator_list):
        ...
        qualifier_master_dict = {
            ...
            PROCESS: {
                ...
                "format_pattern": "format_string_range",
                "add_timestamp_to_relevance": 1,
                "default_if_attr_undefined": f'| "{epoch_time_string}"',
                "os_dependency": 1,
            },
            SOCKET: {
                "mapped_field": ["creation time", "start time"],
        ...
        try:
            if qualifier_master_dict.get(stix_obj).get('add_timestamp_to_relevance'):
                ...
                time_range_tuple = list(time_range_iterator)
                for each in mapped_field:
                    interim_format_string_list = [
                        relevance_map_dict.get('format_string')
                        .get(
                            qualifier_master_dict.get(stix_obj).get(
                                'format_pattern'
                            )
                        )
                        .format(
                            ...
                            default_value_transformer=str_transformer
                            if default_if_attr_undefined
                            else '',
                            comparator=each_operator,
                            start_value=f'"{time_range_tuple[index_op]}"',
                            transformer=str_transformer,
                        )
                        for index_op, each_operator in enumerate(
                            range_operator_list
                        )
                    ]

                    format_string_list.append(f"({' AND '.join(interim_format_string_list)})")
    ...
    def get_field_relevance_qry(self, current_obj, master_obj):
        ...
        """
        if current_obj not in self._relevance_property_format_string_dict.get(
            'object_hierarchy'
        ):
            return
        ...
        for mapped_field in mapped_fields_array:
            ...
            if mapped_field.lower() != SEARCH_FOLDER:
                comparison_string_list.append(self._relevance_qry_list_phrasing(value, comparator, mapped_field,
                                                                                is_negate))
        return self._query_construction_from_list(
            value, comparison_string_list, conditional_attr
        )
            ...
            if stix_obj_attr in attr_marked_for_expr_split:
                if value_type:
                    if is_value_type_mac:
                        ...
                        if stix_obj_attr == attr_for_non_expr_modification:
                            raise ValueError(
                                f"Irrelevant value:{value} for attribute {stix_obj_attr}. Please check from_stix pattern"
                            )

                elif stix_obj_attr == attr_for_non_expr_modification:
                    is_split_expr = True
        else:
    ...
    def __eval_comparison_exp(self, expression):
        ...
        stix_object, stix_field = expression.object_path.split(':')
        conditional_attr = conditional_attr_dict.get(stix_field.split('.')[0])
        ...
        if expression.comparator == ComparisonComparators.In:
            ...
            raise NotImplementedError(
                f"Unknown comparison operator {expression.comparator}."
            )
        ...
        if f'{FILE}.{SEARCH_FOLDER}' in mapped_fields_array:
    ...
    def __eval_obs_exp_subroutine(self, relevance_query, qualifier):
        ...
        if self.qualifier_string:
            ...
            if relevance_query:
                ...
                relevance_query += f' AND {self.qualifier_string}'
            else:
